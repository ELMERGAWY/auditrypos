...
**آخر commit مرفوع:** `cd2bb39 — feat: add appearance settings and table view preferences`
...
| `cd2bb39` | إعدادات مظهر متعددة القوالب، ألوان HSL وكثافة المحتوى، ترتيب البطاقات، طرق عرض الجداول، وترتيب deterministic للعملاء والطلبات وفواتير البيع. |
...
| 14 | `20260815110000_inventory_scope_and_transfer_safety.sql` | عزل المخزون والتحويلات حسب workspace/branch. |
| 15 | `20260815120000_purchase_receipt_costing_bridge.sql` | ربط فواتير الشراء والاستلام وتكلفة المخزون. |
| 16 | `20260815130000_inventory_costing_engine_v2.sql` | محرك التكلفة القابل للتوسعة. |
| 17 | `20260815140000_pos_inventory_costing_bridge.sql` | ربط POS بخصم التكلفة والوصفات. |
| 18 | `20260815150000_inventory_permissions_guard.sql` | ضوابط صلاحيات المخازن والحركات. |
| 19 | `20260815160000_inventory_reconciliation_controls.sql` | مطابقة المخزون وسجل الفروقات. |
| 20 | `20260815170000_service_module_foundation.sql` | أساس الموديول الخدمي والحزم وتكلفة الخدمة. |
| 21 | `20260815180000_product_types_and_packaging_levels.sql` | أنواع الأصناف ومستويات التعبئة كرتونة/عمود/قطعة. |
| 22 | `20260815190000_contracting_project_controls.sql` | نطاق المشاريع والمواقع والبلوكات والأرشفة الآمنة. |
...
### المظهر والقوالب وطرق عرض الجداول

أضيف داخل تبويب الإعدادات الحالي قسم **المظهر وطرق العرض**، ويتيح للمدير اختيار خمسة قوالب لونية، وتخزين ألوان HSL على مستوى النشاط، وتحديد كثافة المحتوى، أسلوب الشريط الجانبي، ترتيب بطاقات لوحة التحكم، وطريقة العرض الافتراضية للجداول. لم يتم إنشاء صفحة موازية؛ القالب يطبّق على Dashboard الحالي، وتستخدم شاشات العملاء والطلبات وفواتير البيع نفس شريط العرض لخيارات الجدول والبطاقات والمضغوط. كما تم تثبيت ترتيب النتائج باستخدام `created_at`/التاريخ ثم `id` عند تحميل القوائم، مع الإبقاء على fallback الحالي عند فشل الشبكة.
...
| Vitest | **49 اختباراً ناجحاً** في 15 ملفاً. |
...
لا يمكن اعتبار النظام معتمداً قانونياً وفق EAS أو IFRS أو US GAAP بمجرد اختيار selector؛ الاعتماد يحتاج chart mapping، سياسات الشركة، مراجعة محاسب، وإغلاق فترات مالية. كما لم يتم تشغيل migrations في قاعدة العميل من داخل هذه الجلسة، ولم يتم تخزين secrets الإنتاج، ولم يتم تنفيذ Meta App Review أو إنشاء حملات مدفوعة نيابة عن العميل. كما يجب تطبيق migrations الثلاث الأخيرة يدوياً بالترتيب 20260815170000 ثم 20260815180000 ثم 20260815190000 بعد التحقق من عدم وجود الكائنات المطلوبة مسبقاً.
