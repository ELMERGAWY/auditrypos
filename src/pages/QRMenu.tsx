...
import { getRestaurant, addWaiterCall, type MenuItem } from '@/lib/store';
...
const QRMenu = () => {
  ...
  useEffect(() => {
    ...
    if (r) {
      setItems(r.menuItems.filter(i => i.available));
    ...
    <div className="min-h-screen bg-background pb-24" dir="rtl">
      ...
      <div className="gradient-bg p-6 pb-10 rounded-b-3xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="font-display text-xl font-bold text-primary-foreground">{restaurantName || 'القائمة'}</h1>
            <p className="text-sm text-primary-foreground/70">قائمة الطعام الرقمية</p>
          </div>
        </div>
        <div className="relative">
          ...
          <Input
            ...
            className="pr-10 bg-background/90 border-0"
          />
        </div>
      </div>
        ...
        {filtered.map((item, i) => (
          ...
          >
            ...
            <div className="text-left">
              <p className="font-display font-bold text-primary">{item.price} ج.م</p>
            </div>
          </motion.div>
        ))}
        {filtered.length === 0 && (
      ...
      <motion.button
        whileTap={{ scale: 0.9 }}
        onClick={callWaiter}
        className="floating-btn flex items-center gap-2"
      >
        <Bell className="w-6 h-6" />
        <span className="font-bold text-sm">استدعاء ويتر</span>
      </motion.button>
    </div>
  );
};
