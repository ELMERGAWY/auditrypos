...
function WarehousesManager({ restaurantId, warehouses, onRefresh }: { restaurantId: string; warehouses: Warehouse[]; onRefresh: () => void }) {
  ...
  const [form, setForm] = useState({ name: '', code: '', type: 'main', location: '' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setForm({ name: '', code: '', type: 'main', location: '' }); setShowForm(true); }} size="sm" className="gradient-bg text-primary-foreground border-0 rounded-xl">
          ...
          <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass-card p-6 max-w-sm w-full space-y-4 rounded-3xl shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                <Input placeholder="كود المخزن (اختياري)" value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} className="h-11 rounded-xl" />
