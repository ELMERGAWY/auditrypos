...
CREATE POLICY "Users can view hourly rates for their restaurant"
    ...
    USING (restaurant_id = (SELECT restaurant_id FROM staff_profiles WHERE user_id = auth.uid()));

CREATE POLICY "Super admin can view all hourly rates"
    ON marketing_hourly_rates FOR SELECT
    USING (EXISTS (SELECT 1 FROM staff_profiles WHERE user_id = auth.uid() AND is_super_admin = true));

CREATE POLICY "Users can insert hourly rates for their restaurant"
    ON marketing_hourly_rates FOR INSERT
    WITH CHECK (restaurant_id = (SELECT restaurant_id FROM staff_profiles WHERE user_id = auth.uid()));

CREATE POLICY "Super admin can insert hourly rates"
    ON marketing_hourly_rates FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM staff_profiles WHERE user_id = auth.uid() AND is_super_admin = true));
