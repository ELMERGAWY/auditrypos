-- ================================================================
-- MANUAL SQL: Apply this in Supabase Dashboard > SQL Editor
-- ================================================================
-- This file must be run manually because supabase CLI link is not available.
-- Go to: https://supabase.com/dashboard/project/nmkjyweoagbblkbqavdz/sql
-- Paste this entire script and click Run.
-- ================================================================

-- Drop old function signature if exists
DROP FUNCTION IF EXISTS public.execute_inventory_transfer(UUID, UUID, UUID, UUID, NUMERIC, TEXT);
...
BEGIN
  ...
  IF v_dst_product_id IS NULL THEN
    INSERT INTO public.products (
      restaurant_id, warehouse_id, name, barcode, sku, category,
      price, cost_price, unit, secondary_unit, unit_conversion_factor,
      image, quantity, min_quantity, available
    ) VALUES (
      p_restaurant_id, p_to_warehouse_id, v_src_name, v_src_barcode, v_src_sku,
      v_src_category, v_src_price, v_src_cost_price, v_src_unit, v_src_sec_unit,
      v_src_conv_factor, v_src_image, 0, 5, true
    )
    RETURNING id, unit, secondary_unit, COALESCE(unit_conversion_factor, 1)
    INTO v_dst_product_id, v_dst_unit, v_dst_sec_unit, v_dst_conv_factor;
  END IF;

  -- 5. Calculate UOM conversion ratio
  ...
  INSERT INTO public.inventory_transfers (
    restaurant_id, from_warehouse_id, to_warehouse_id, notes, status
  ) VALUES (
    p_restaurant_id, p_from_warehouse_id, p_to_warehouse_id, p_notes, 'received'
  )
  RETURNING id INTO v_transfer_id;

  -- 7. Create inventory_transfer_items record
  INSERT INTO public.inventory_transfer_items (
    transfer_id, restaurant_id, product_id, quantity, cost_price
  ) VALUES (
    v_transfer_id, p_restaurant_id, p_product_id, p_quantity, COALESCE(v_src_cost_price, 0)
  );

  -- 8. Deduct from source product quantity
  UPDATE public.products
  SET quantity = GREATEST(0, COALESCE(quantity, 0) - p_quantity)
  WHERE id = p_product_id;

  -- 9. Add to destination product quantity
  UPDATE public.products
  ...
  ELSE
    ...
    VALUES (p_restaurant_id, p_from_warehouse_id, p_product_id, 0)
    ON CONFLICT (warehouse_id, product_id)
    DO UPDATE SET quantity = GREATEST(0, public.warehouse_stock.quantity - p_quantity);
  END IF;

  -- 11. Add to destination warehouse_stock (THE BUG FIX - was missing destination update)
  SELECT id, quantity INTO v_dst_stock_id, v_dst_qty
  FROM public.warehouse_stock
  WHERE warehouse_id = p_to_warehouse_id AND product_id = v_dst_product_id
  LIMIT 1;
  ...
  ELSE
    INSERT INTO public.warehouse_stock (restaurant_id, warehouse_id, product_id, quantity)
    VALUES (p_restaurant_id, p_to_warehouse_id, v_dst_product_id, v_dst_transfer_qty)
    ON CONFLICT (warehouse_id, product_id)
    DO UPDATE SET quantity = public.warehouse_stock.quantity + v_dst_transfer_qty;
  END IF;
...
SELECT 'execute_inventory_transfer function created/updated successfully!' AS status;
